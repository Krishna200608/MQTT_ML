...
if 0 in flow_list_dict and len(flow_list_dict[0]) > 0:
...
for key in flow_list_dict:
    ...
    if not flow_list: # Skip if flow_list is empty
        continue
...
if output_uniflows_separately:
    # --- START PANDAS 2.0 FIX 1 ---
    # Create a list to hold DataFrames, then concat once at the end.
    uniflow_list_for_df = []
    
    for flow in meta_list:
        flow_df = uniFlow2df(flow)
        uniflow_list_for_df.append(flow_df)
        
    # Concat all DataFrames in the list into the final feature_df
    feature_df = pd.concat(uniflow_list_for_df, ignore_index=True, sort=False)
...
# --- START PANDAS 2.0 FIX 2 & 3 ---
# Create an empty list to hold the biflow DataFrames
biflow_list_for_df = []
# The original script created an empty df_biflow here, which we no longer need.
# --- END PANDAS 2.0 FIX ---
    ...
    if (current.processed == False):
        ...
        for inner_row in range(row_num, num_uniflows):
            ...
            if (rev_ip_src == inner_ip_src) and (rev_ip_dst == inner_ip_dst) and (rev_prt_src == inner_prt_src) and (rev_prt_dst == inner_prt_dst) and (proto == inner_proto):
                ...
                # --- START PANDAS 2.0 FIX 2 ---
                biflow_list_for_df.append(pd.DataFrame([biflowlist], columns = columns_list))
                # --- END PANDAS 2.0 FIX 2 ---
            else:
                continue
    else:
        continue
...
for row_num in range(num_uniflows):
    ...
    if (current.processed == False):
        ...
        if proto == 17:
            ...
            # --- START PANDAS 2.0 FIX 3 ---
            biflow_list_for_df.append(pd.DataFrame([biflowlist], columns = columns_list))
            # --- END PANDAS 2.0 FIX 3 ---
        else:
            continue

# --- START PANDAS 2.0 FIX: Concat all biflows at the end ---
df_biflow = pd.concat(biflow_list_for_df, ignore_index=True, sort=False)
# --- END PANDAS 2.0 FIX ---
...
# --- FIX: Initialize column as float to prevent FutureWarning ---
df_biflow['src_ip_dst_prt_delta'] = 0.0
# --- END FIX ---
...
# Handle potential empty df_biflow
if not df_biflow.empty:
